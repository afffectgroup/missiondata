...
const LOG_COLORS = { i:'#7DD3FC', s:'#86EFAC', e:'#FCA5A5', c:'#C4B5FD' }

/* ── GenerationProgress — shown while status === 'generating' ── */
  ...
  return (
    <div className="card" style={{ padding: 24, marginBottom: 20 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
        ...
        <span style={{ fontFamily:'var(--fd)', fontSize:15, fontWeight:700, color:'var(--text)' }}>
          Pipeline en cours…
        </span>
      ...
      <div ref={logRef} style={{
        ...
        fontFamily:'var(--fm)', fontSize:12, height:220, overflowY:'auto', lineHeight:1.8,
      }}>
        {(logs || []).map((l, i) => (
...
function VolumeEstimate({ campaignId }) {
  ...
  if (done && count !== null) {
    return (
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', background:'var(--bg2)', borderRadius:'var(--r-md)', border:'1px solid var(--border3)' }}>
        <span style={{ fontFamily:'var(--fd)', fontSize:20, fontWeight:800, color:'var(--mf-blue)' }}>{count.toLocaleString('fr-FR')}</span>
        <span style={{ fontSize:13, color:'var(--t2)' }}>profils correspondent à vos critères</span>
        <span style={{ fontFamily:'var(--fm)', fontSize:10, color:'var(--t4)', marginLeft:4 }}>· gratuit · 0 crédit</span>
      </div>
    )
  }
  ...
  return (
    <button className="btn btn-secondary btn-sm" onClick={estimate} disabled={loading}>
      {loading
        ? <><span className="spinner" style={{ width:13, height:13 }} /> Estimation...</>
        : '◎ Estimer le volume (gratuit)'}
    </button>
  )
}
...
function SequenceCard({ seq, prospect, onRegenerate }) {
  ...
  const [content, setContent] = useState(seq[tab] || '')
  const [editing, setEditing] = useState(false)
  const [regen, setRegen] = useState(false)

  useEffect(() => setContent(seq[tab] || ''), [tab, seq])
  ...
  const handleRegen = async () => {
    setRegen(true)
    await onRegenerate(seq.prospect_id)
    setRegen(false)
  }
    ...
    <div className="card" style={{ overflow:'hidden' }}>
      <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          {prospect?.email && (
            <span style={{ fontFamily:'var(--fm)', fontSize:11, color:'var(--ab)' }}>{prospect.email}</span>
          )}
          <button className="btn btn-secondary btn-sm" onClick={handleRegen} disabled={regen}>
      ...
      <div style={{ padding:20 }}>
        {editing ? (
          <>
            <textarea className="textarea" value={content} onChange={e => setContent(e.target.value)} style={{ minHeight:160, fontSize:13, lineHeight:1.7 }} />
            <div style={{ display:'flex', gap:8, marginTop:10 }}>
              <button className="btn btn-primary btn-sm" onClick={() => setEditing(false)}>Sauvegarder</button>
              <button className="btn btn-secondary btn-sm" onClick={() => { setEditing(false); setContent(seq[tab]) }}>Annuler</button>
            </div>
          </>
        ) : (
          <>
...
export default function CampaignDetail() {
  ...
  function subscribeRealtime() {
    supabase
      .channel(`campaign-progress-${id}`)
      .on('postgres_changes', {
        event: 'UPDATE', schema: 'public', table: 'campaigns', filter: `id=eq.${id}`
      }, payload => {
        setCampaign(prev => prev ? { ...prev, ...payload.new } : payload.new)
        // When done, refresh prospects
        if (payload.new.status === 'done' && payload.new.generation_pct === 100) {
          fetchProspects()
        }
      })
      .subscribe()
    ...
    if (r.ok) {
      ...
      setSequences(prev => {
        ...
        for (const s of (d.sequences || [])) {
          const idx = updated.findIndex(x => x.prospect_id === s.prospect_id)
  ...
  const seqMap = Object.fromEntries(sequences.map(s => [s.prospect_id, s]))
  ...
  return (
    <>
      ...
      <Layout title={campaign.name} action={<Link href="/dashboard"><button className="btn btn-secondary btn-sm">← Dashboard</button></Link>}>
        ...
        {/* ── Real-time progress ── */}
        {isGenerating && (
          <GenerationProgress pct={campaign.generation_pct || 0} logs={campaign.generation_logs || []} />
        )}
        ...
        {!isGenerating && (
          <div style={{ display:'flex', gap:12, marginBottom:24, padding:'14px 18px', background:'var(--white)', borderRadius:'var(--r-lg)', border:'1px solid var(--border)', boxShadow:'var(--sh1)', flexWrap:'wrap', alignItems:'center' }}>
            ...
            {prospects.length > 0 && (
              <button className="btn btn-secondary" onClick={generateSequences} disabled={genSeq}>
                {genSeq ? <><span className="spinner" style={{ width:14, height:14 }} /> Génération…</> : '✦ Générer les séquences'}
              </button>
            )}
          ...
          <>
            ...
            {tab === 'prospects' && (
              <div className="fade-in">
                {prospects.length === 0 ? (
                  <div className="card"><div className="empty"><div className="empty-icon">◎</div><h3>Aucun prospect</h3><p>Cliquez sur "Trouver des prospects" pour lancer la recherche.</p></div></div>
                ) : (
                  <div className="card" style={{ overflow:'hidden' }}>
                    <table className="table">
                      <thead><tr><th>Nom</th><th>Poste</th><th>Entreprise</th><th>Email</th><th>Certitude</th><th>Statut</th><th>LinkedIn</th></tr></thead>
                      <tbody>
                        {prospects.map(p => {
                          const cert = CERT_MAP[p.email_cert]
                          const kanbanLabels = { applied:'Candidaté', followed_up:'Relancé', in_progress:'En cours', signed:'Signé ✓' }
                          return (
                            <tr key={p.id}>
                              ...
                              <td>
                                {p.kanban_status
                                  ? <span className="badge badge-live">{kanbanLabels[p.kanban_status]}</span>
            ...
            {tab === 'sequences' && (
              <div className="fade-in">
                {sequences.length === 0 ? (
                  <div className="card"><div className="empty"><div className="empty-icon">✦</div><h3>Aucune séquence</h3><p>{prospects.length === 0 ? 'Trouvez d\'abord des prospects.' : 'Cliquez sur "Générer les séquences".'}</p></div></div>
                ) : (
              ...
              <div className="fade-in card" style={{ padding:28 }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                  {[
                    ...
                    { l:'Ton', v:campaign.freelance_tone }, { l:'KPI', v:campaign.freelance_kpi },
                  ].map(r => (
